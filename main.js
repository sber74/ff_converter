    ...
    switch (key) {
      case START_TRANS:
        ...
        if (Object.keys(newTrans).length) {
          if (listSplitTrans.length) {
            newTrans.listSplitTrans = listSplitTrans;
          }
      ...
      case "S": //SPLIT-CATEGORY
        ...
        splitTrans = {};
        splitTrans.category = value;
        break;

      case "E": //SPLIT-DESCRIPTION
        splitTrans.description = value;
        break;

      case "$": //SPLIT-AMOUNT
        splitTrans.amount = Number(value.replaceAll(",", ""));
        listSplitTrans.push(splitTrans);
        break;
...
function transformTrans(trans) {
  try {
    ...
    // Traitement des transactions SPLIT
    if (trans?.tags && trans.tags.includes("SPLIT")) {
      if (trans?.listSplitTrans && trans.listSplitTrans.length > 0) {
        ...
        throw new Error("Opération splittée incohérente");
      }
    } else {
      transCSV.split = "";
    }
